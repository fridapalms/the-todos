...
if (todosFromLs !== null) { //om listan i localStorage finns
  todos = JSON.parse(todosFromLs); //ska originallistan ersättas med listan från localStorage
} 

if (todos.length === 0) { //om längden på listan är 0, dvs inga punkter i listan
  todos = [                // ersätts listan med placeholder-punkter enligt nedan
...
sort.addEventListener("click", () => { //lyssnar efter klick på sort-img
  ...
  if (reversed) { //om listan är reversed (true) (nerifrån och upp)
    sort.src = "/src/img/sort2.png"; //använd denna ikon
  } else { //om inte (reverse är false, listan är uppifrån och ner)
...
if (orderFromLs !== null) { //kontrollerar att det finns något sparat i localstorage
